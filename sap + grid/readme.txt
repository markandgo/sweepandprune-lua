...
-- Create a new instance and subdivide it into cells with the given width and height
-- Recommended cell size to be at least as big as the average size of objects
-- Note that each cell has its own SAP instance
...
-- Only call this method once before each update!
sapgridA:move(id,x0,y0,x1,y1)

-- Delete the AABB for the given ID. This method has priority over add and move for each update
sapgridA:delete(id)

-- Update the instance. Note that the other methods are 'buffered' until update is called.
sapgridA:update()
